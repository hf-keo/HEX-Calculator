...
export function BeamModule() {
  ...
  const set = <K extends keyof BeamInput>(k: K, v: number) =>
  ...
  return (
    <div style={{ border: "1px solid #ddd", borderRadius: 12, padding: 14 }}>
      <h2 style={{ marginTop: 0 }}>Beam / I-Beam Cracking Calculator</h2>
      <div style={{ opacity: 0.8, marginBottom: 10 }}>
        Fill inputs (EN + Metric side-by-side), then click <b>Calculate</b>.
      </div>

      <h3>Geometry</h3>
      <DualNumberField
        label="Lever radius R (center → load point)"
        enUnit="in"
        siUnit="mm"
        baseValue={inp.leverRadius_in}
        setBaseValue={(v) => set("leverRadius_in", v)}
        fromBaseToEN={(x) => x}
        fromBaseToSI={in_to_mm}
        toBaseFromEN={(x) => x}
        toBaseFromSI={mm_to_in}
        decimals={3}
      />

      <DualNumberField
        label="Support span L (left → right support)"
        enUnit="in"
        siUnit="mm"
        baseValue={inp.supportSpan_in}
        setBaseValue={(v) => set("supportSpan_in", v)}
        fromBaseToEN={(x) => x}
        fromBaseToSI={in_to_mm}
        toBaseFromEN={(x) => x}
        toBaseFromSI={mm_to_in}
      ...
      <DualNumberField
        label="Load position a (from left support)"
        enUnit="in"
        siUnit="mm"
        baseValue={inp.loadPosFromLeft_in}
        setBaseValue={(v) => set("loadPosFromLeft_in", v)}
        fromBaseToEN={(x) => x}
        fromBaseToSI={in_to_mm}
        toBaseFromEN={(x) => x}
        toBaseFromSI={mm_to_in}
      ...
      {inp.supportSpan_in > 0 && inp.loadPosFromLeft_in > inp.supportSpan_in && (
        <div style={{ marginBottom: 12, color: "#b00020" }}>
          Load position a must be ≤ support span L.
        </div>
      )}

      <h3>Load</h3>
      <DualNumberField
        label="Applied force F (line pull / jack force)"
        enUnit="lbf"
        siUnit="kN"
        baseValue={inp.appliedForce_lbf}
        setBaseValue={(v) => set("appliedForce_lbf", v)}
        fromBaseToEN={(x) => x}
        fromBaseToSI={lbf_to_kN}
        toBaseFromEN={(x) => x}
        toBaseFromSI={kN_to_lbf}
        decimals={2}
      />

      <h3>Beam Properties</h3>
      <DualNumberField
        label="Young’s modulus E (steel ~ 29,000,000 psi)"
        enUnit="psi"
        siUnit="MPa"
        baseValue={inp.E_psi}
        setBaseValue={(v) => set("E_psi", v)}
        fromBaseToEN={(x) => x}
        fromBaseToSI={psi_to_MPa}
        toBaseFromEN={(x) => x}
        toBaseFromSI={MPa_to_psi}
        decimals={0}
      />
      ...
      <DualNumberField
        ...
        enUnit="psi"
        siUnit="MPa"
        baseValue={inp.allowableBendingStress_psi}
        setBaseValue={(v) => set("allowableBendingStress_psi", v)}
        fromBaseToEN={(x) => x}
        fromBaseToSI={psi_to_MPa}
        toBaseFromEN={(x) => x}
        toBaseFromSI={MPa_to_psi}
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
        ...
        <button
          onClick={() => { setInp(emptyBeam); setOut(null); }}
        ...
        <div style={{ marginTop: 16, borderTop: "1px solid #eee", paddingTop: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={{ border: "1px solid #ddd", borderRadius: 10, padding: 12 }}>
              ...
              <div>
                <b>Torque:</b> {out.torque_ftlbf.toFixed(0)} ft·lbf{" "}
                ({ftlbf_to_Nm(out.torque_ftlbf).toFixed(0)} N·m)
              </div>
            </div>
            ...
            <div style={{ border: "1px solid #ddd", borderRadius: 10, padding: 12 }}>
              ...
              <div>
                ...
                <span style={{ marginLeft: 6 }}>
                  {utilStatus?.icon} {utilStatus?.label}
                </span>
              </div>
            </div>
